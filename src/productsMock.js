export const products = [

    {
        id: 1,
        title: "Cien años de soledad",
        price: 7500,
        author: "Gabriel García Márquez",
        description: "Entre la boda de José Arcadio Buendía con Amelia Iguarán hasta la maldición de Aureliano Babilonia transcurre todo un siglo. Cien años de soledad para una estirpe única, fantástica, capaz de fundar una ciudad tan especial como Macondo y de engendrar niños con cola de cerdo.",
        genero: "Ficcion",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689033277/medium_ixgmdo.jpg"
    },
    {
        id: 2,
        title: "La mano izquierda de la oscuridad",
        price: 9500,
        author: "Ursula K. Le Guin",
        description: "La luz es la mano izquierda de la oscuridad, y la oscuridad es la mano derecha de la luz. Las dos son una, vida y muerte, juntas como amantes en kémmer, como manos unidas, como el término y el camino",
        genero: "Ficcion",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689033315/portada_la-mano-izquierda-de-la-oscuridad_ursula-k-le-guin_202006041508_cnzmbe.webp"
    },
    {
        id: 3,
        title: "It",
        price: 10000,
        author: "Stephen King",
        description: "Cuenta la historia, en un juego de correspondencias con el pasado y el presente, de un grupo de siete amigos que son perseguidos por una entidad sobrenatural, al que llaman «Eso», que es capaz de cambiar de forma y se alimenta principalmente del terror que produce en sus víctimas.",
        genero: "Terror",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689033384/2e004a40fd5c55013d75b50fa02a951e_ev4fmx.jpg"
    },
    {
        id: 4,
        title: "Involución",
        price: 15000,
        author: "Max Brooks",
        description: "Greenloop fue, hasta la inesperada erupción del Rainier, una selecta comunidad ecológica. Ahora, de entre los escombros y los rastros de sangre de la urbanización, han podido recuperarse los diarios de Kate Holland.En ellos se relata una historia demasiado espantosa para ser olvidada, tan horrenda que podría romper nuestras convicciones.Incluso las más firmes, como la solidez de nuestra civilización",
        genero: "Terror",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689033191/9788417910860_jhiaqe.jpg"
    },

    {
        id: 5,
        title: "El prisionero de Zenda",
        price: 9500,
        author: "Anthony Hope",
        description: "Rudolf Rassendyll es un joven y adinerado noble que vive apaciblemente  en Inglaterra sin otra preocupación que su propio entretenimiento.Cuando decide viajar al reino de Ruritania para asistir a la coronación  del rey Rudolf V, poco sospecha que se enfrentará a un reto mayor del que jamás hubiese podido soñar.",
        genero: "Aventura",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689033244/El-prisionero-de-Zenda-154x233_xszfat.jpg"
    },
    {
        id: 6,
        title: "Los tres mosqueteros",
        price: 8500,
        author: "Alejandro Dumas",
        description: "D'Artagnan, un joven gascón de familia noble venida a menos, parte a París para cumplir su sueño de convertirse en Mosquetero. Allí entabla una fuerte amistad con tres de ellos: Athos, Porthos y Aramis. Juntos se verán envueltos en todo tipo de peripecias, romances e intrigas políticas en contra del Cardenal Richelieu",
        genero: "Aventura",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689033128/9788469808412-los-tres-mosqueteros-clasicos-a-medida_zo12ab.jpg"
    },
    {
        id: 7,
        title: "Viaje al centro de la Tierra",
        price: 11000,
        author: "Julio Verne",
        description: "El joven Axel y su tío, el profesor Otto, emprenden un viaje fantástico al interior de la Tierra. Y todo ello a partir del descubrimiento de unos signos misteriosos hallados entre las páginas de un libro antiguo",
        genero: "Aventura",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689033062/19302624155_c3zum4.jpg"
    },
    {
        id: 8,
        title: "Estudio en escarlata",
        price: 7500,
        author: "Arthur Conan Doyle",
        description: "Un cadáver hallado en extrañas circunstancias pone en marcha los reflejos deductivos de Holmes, mientras la policía oficial se pierde en divagaciones equivocadas o arresta a inocentes ciudadanos.",
        genero: "Policial",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689032934/625452f9a6137d58b8b112d5_6034d7d1f3e0f5af57b2b268_Estudio-en-escarlata-el-signo-de-los-cuatro-arthur-conan-doyle-editorial-alma_ogxh5m.jpg"
    },
    {
        id: 9,
        title: "La Dalia Negra",
        price: 10500,
        author: "James Ellroy",
        description: "El 15 de enero de 1947, en un solar de Los Ángeles, apareció el cadáver desnudo y seccionado en dos de una mujer joven. El médico forense determinó que la habían torturado durante días. Elizabeth Short, de 22 años, llamada la Dalia Negra, llevará a los detectives a los bajos fondos de Hollywood, para así involucrar a ciertas personas adineradas de Los Ángeles. Ambos están obsesionados por lo que fue la vida de la Dalia Negra, y, sobre todo, por capturar al individuo que la asesinó.",
        genero: "Policial",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689032874/9788439729297_tv2tsy.jpg"
    },
    {
        id: 10,
        title: "Nosotros en la luna",
        price: 7500,
        author: "Alice Kellen",
        description: "Cuando Rhys y Ginger se conocen en las calles de la ciudad de la luz, no imaginan que sus vidas se unirán para siempre, a pesar de la distancia y de que no puedan ser más diferentes. Ella vive en Londres y a veces se siente tan perdida que se ha olvidado hasta de sus propios sueños. Él es incapaz de quedarse quieto en ningún lugar y cree saber quién es. Y cada noche su amistad crece entre emails llenos de confidencias, dudas e inquietudes. Pero ¿qué ocurre cuando el paso del tiempo pone a prueba su relación? ¿Es posible colgarse de la luna junto a otra persona sin poner en riesgo el corazón?",
        genero: "Romantico",
        img: "https://res.cloudinary.com/dottjemxj/image/upload/v1689032780/310721_portada_nosotros-en-la-luna_alice-kellen_201911261050_cmh0gy.jpg"
    },

]